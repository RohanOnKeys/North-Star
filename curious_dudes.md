# North Star, Explained for Curious Dudes Who Don't Know Shii

## The extremely short version

**North Star** is an experimental networking project for moving data between
orbital computers and Earth.

North Star explores this question:
...
horizon, communicates for a few minutes, disappears, and comes back later.
North Star is designed to continue working through that mess.
...
That is the territory North Star explores.
...
North Star divides data into chunks, remembers which chunks arrived, stores
the rest, and continues during another contact window.
...
+----------------+                   +----------------+
| North Star node|  <--- link ---->  | North Star node|
...
North Star includes four representative Low Earth Orbit satellites described
using static **Two-Line Element sets**, usually called TLEs.
...
horizon. For each pass, North Star calculates:
...
North Star supports several workload types:
...
North Star also occasionally serves lower-priority data so that a model
transfer cannot be ignored forever.
...
### 5. Every chunk travels inside a North Star frame

A North Star frame contains:
...
The simulator encrypts payloads with a small educational construction based
on HMAC-generated bytes and XOR.
...
North Star now has a **LIVE-SIM** made of three standalone programs:
...
during one real second. North Star scales bandwidth by the same factor so each
shortened pass keeps roughly the same total transfer capacity.
...
North Star borrows the general store-and-forward mindset, but it is deliberately much
smaller:

| North Star today | Bundle Protocol |
|---|---|
...
North Star is therefore not claiming, “We invented space networking.”
...
AFSK1200 and APT return a clear **not implemented** result. North Star does not
pretend that recognizing a test tone means it can decode the ISS or a weather
satellite.
...
Keeping the programs separate prevents North Star from assuming every orbital
channel is slow VHF audio. A future link can be radio or optical while the
chunking and store-and-forward logic above it remains the same.
...
At the moment, “secure” means that North Star demonstrates:
...
Before North Star could protect real spacecraft, it would need:
...
Run North Star across those passes:
...
North Star has completed a real three-process LIVE-SIM transfer with:
...
For every 32,768-byte DATA chunk, the current live protocol adds:
...
So the North Star application framing overhead is about 0.244% at the current
...
### Is North Star faster than HTTP/3, QUIC, or Bundle Protocol?

We do not know yet.

North Star has **not** been benchmarked against production HTTP/3, QUIC, or
BPv7 implementations under the same pass schedule and packet-loss trace.
Claiming a percentage speed advantage would be made-up marketing.

What has been demonstrated is different:

| North Star strength today | Meaning |
|---|---|
...
| 99.756% app framing efficiency | Large chunks spend little space on North Star headers |
...
requests, ETags, and multipart APIs. HTTP/3 and QUIC have far more mature
security, congestion control, multiplexing, and continuous-link performance.
BPv7 is a real delay-tolerant networking standard with multi-hop forwarding,
lifetimes, extension blocks, and interoperable security.

North Star's honest current claim is:
...
North Star is not currently “the future replacement for HTTP.”
...
The project combines orbital pass prediction with a small, inspectable
protocol that can prioritize useful work, survive interruption, and resume
later.

That is North Star: not magic space internet yet, but a working laboratory for
figuring out what magic space internet might require.
